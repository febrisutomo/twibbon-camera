  ...
  const capturePhoto = useCallback(async () => {
    ...
    if (selectedTwibon) {
      await new Promise<void>((resolve) => {
        ...
        twibonImg.onload = () => {
          ctx.drawImage(twibonImg, 0, 0, canvasWidth, canvasHeight);
          resolve();
        };
        twibonImg.onerror = () => resolve();
      ...
      <div className="relative container h-full px-4 py-4">
        ...
        >
          <video
            ...
            muted
            playsInline
            className={`w-full h-full object-cover transition-opacity duration-300 ${isCameraReady ? 'opacity-100' : 'opacity-0'} ${facingMode === 'user' ? 'scale-x-[-1]' : ''}`}
          />
